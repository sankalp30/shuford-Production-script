...
#%%load dataframes
path_cno = 'H:/costing_automation/tables_marzolispinning'
df_dsms_cno = pd.read_excel(path_cno + '/dsms_cno_tbl.xlsx')

df_dsms_allowance = pd.read_excel(path_cno + '/dsms_allowance_tbl.xlsx')

df_dsms_spinnertimes = pd.read_excel(path_cno + '/dsms_spinnertime_tbl.xlsx')

df_dsms_windtimes = pd.read_excel(path_cno + '/dsms_windertime_tbl.xlsx')

df_dsms_blenddetail = pd.read_excel(path_cno + '/dsms_blendbobbinwt_tbl.xlsx')

df_dsms_hankrov = pd.read_excel(path_cno + '/dsms_rovingbobbin_tbl.xlsx')
#%%
'''
...
class ReadData:
    def __init__(self, path, cno_file, allowance_file, spintime_file, windtime_file, blenddetail_file, hankrov_file):
        ...
        self.df_spinnertimes = pd.read_excel(path + spintime_file +'.xlsx')
        self.df_windtimes = pd.read_excel(path_cno + windtime_file +'.xlsx')
        self.df_blenddetail = pd.read_excel(path_cno + blenddetail_file + '.xlsx')
        self.df_hankrov = pd.read_excel(path_cno + hankrov_file +'.xlsx')
...
#%%     
class dsms_spinning_winding(dsms_data):   
    """
    metrics calculating class from spinning and winding
    """
    
    def __init__(self,*args, **kwargs):
        super(dsms_spinning_winding, self).__init__(*args, **kwargs)
    ...
    def cycletime_spin(self):
        return ((self.runtime_spin() + \
                ...
                2*dsms_data.spin_delay*self.spinner_breaks*self.runtime_spin()/self.numspindles_spin)*\
...
#%% 
class dsms_line(dsms_spinning_winding):
    """
    model class for metrics calculation for combined spinning and winding systems.
    """
    
    def __init__(self, *args, **kwargs):
        super(dsms_line, self).__init__(*args, **kwargs)
    
    def runtime_line(self):
        return max(dsms_spinning_winding.runtime_spin(self), dsms_spinning_winding.runtime_windbatch(self))
    
    def bottleneckshift_delay(self):
        a = dsms_spinning_winding.cycletime_spin(self)
        b = dsms_spinning_winding.cycletime_windbatch(self)
        if b>a:    
            return dsms_spinning_winding.cycletime_wind(self)*self.bobbin_weight*592/(self.pkg_weight*self.w_numspindles)
        else:
            return 0
    
    def cycletime_line(self):
        return max(dsms_spinning_winding.cycletime_spin(self), dsms_spinning_winding.cycletime_windbatch(self) + self.bottleneckshift_delay())
    
    def opt_line(self):
        return (dsms_spinning_winding.opt_spin(self) + dsms_spinning_winding.opt_windbatch(self))
    
    def maxlb_line(self):
        return min(dsms_spinning_winding.maxlb_spin(self), dsms_spinning_winding.maxlb_wind(self))
    
    def explb_line(self):
        return min(dsms_spinning_winding.explb_spin(self), dsms_spinning_winding.explb_wind(self))
    
    def efficiency_line(self):
        return self.runtime_line()/self.cycletime_line()
    
    def stdminperlb_line(self):
        return (dsms_spinning_winding.opt_spin(self) + dsms_spinning_winding.opt_windbatch(self)*self.w_numspindles)*480/(430*self.bobbin_weight*self.numspindles_spin)
    
    def optassign_line(self):
        return (430/self.opt_line())/(480/(self.cycletime_line()))
    
     
#%%TEST Case - Do not run
trycno = dsms_cno(229603, df_dsms_cno)     
blend_detail = dsms_blend( trycno.cno_present['blend_type'].values[0], df_dsms_blenddetail)
spintime = dsms_spintime(df_dsms_spinnertimes)
rov_weight = dsms_rovingweight(trycno.cno_present['hank_roving'].values[0], df_dsms_hankrov)

spin_data = dsms_line(rov_weight, trycno.cno_present, blend_detail.blend_detail, spintime.spintimes, df_dsms_windtimes, df_dsms_allowance)

print(spin_data.runtime_line(), spin_data.bottleneckshift_delay(), spin_data.cycletime_line(), spin_data.opt_line(), \
      spin_data.maxlb_line(), spin_data.explb_line(), spin_data.efficiency_line(), spin_data.stdminperlb_line(), spin_data.op_assign_spin(), spin_data.op_assign_wind(), spin_data.optassign_line())
...
for i, row in df_dsms_cno.iterrows():
    cno_x = df_dsms_cno.iloc[i]['cno_1']
    currentcno = dsms_cno(cno_x, df_dsms_cno)
    blend_detail = dsms_blend(currentcno.cno_present['blend_type'].values[0], df_dsms_blenddetail)
    spintime = dsms_spintime(df_dsms_spinnertimes)
    rov_weight = dsms_rovingweight(currentcno.cno_present['hank_roving'].values[0], df_dsms_hankrov)
    
    print(rov_weight, currentcno.cno_present['cno_1'].values[0], blend_detail.blend_detail)
    
    spin_data = dsms_line(rov_weight, currentcno.cno_present, blend_detail.blend_detail, spintime.spintimes, df_dsms_windtimes, df_dsms_allowance)
    ...
    x= pd.DataFrame([[currentcno.cno_present['cno_1'].values[0], currentcno.cno_present['yno'].values[0], currentcno.cno_present['blend'].values[0], \
                      ...
                      spin_data.maxlb_line(), spin_data.explb_line(), spin_data.efficiency_line(), spin_data.stdminperlb_line(), spin_data.op_assign_spin(), spin_data.op_assign_wind(), spin_data.optassign_line()]], columns = cols)
...
#%%
out_path = 'H:\costing_automation\operator_assigment.xlsx'
